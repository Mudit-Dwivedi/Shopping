...
//     e.preventDefault();
//     try {
//       const response = await fetch('https://shopping-z604.onrender.com/user/login', {
//         method: 'POST',
//         headers: {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('https://shopping-z604.onrender.com/user/login', {
        ...
        body: JSON.stringify({ email, password }),
        credentials: 'include', // Include cookies, if needed (for tokens stored in cookies)
      });
      ...
      if (data.success) {
        ...
        // Optionally: Save the wishlist if available
        if (Array.isArray(data.wishlist)) {
          localStorage.setItem('wishlist', JSON.stringify(data.wishlist));
          setWishlist(data.wishlist);  // Make sure this function is available from context
          console.log("Wishlist saved:", data.wishlist); // Debug wishlist
        ...
        if (token) {
          fetch('https://shopping-z604.onrender.com/some-protected-endpoint', {
            ...
            .then(protectedData => {
              console.log('Protected data fetched successfully:', protectedData);
            })
